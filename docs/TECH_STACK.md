...
| pdf-parse                             | PDF text extraction          | Wraps pdfjs for `/api/extract`. Lazy-imported inside the PDF branch and listed in `serverExternalPackages` - see the note below                                                                                                          |
| @napi-rs/canvas                       | DOMMatrix/ImageData polyfill | pdfjs evaluates `new DOMMatrix()` at module scope and only self-polyfills via a runtime `createRequire` that Next's tracer cannot see. Declared directly and imported explicitly in `/api/extract` so the package reaches the deployment |
