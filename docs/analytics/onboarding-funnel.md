...
| `onb_flow_dismissed` | `step`                      | HISTORICAL (2026-07-31..2026-08-19): modal closed via X / Escape / outside click. The `onb-modal-exit` experiment concluded ship-locked (docs/experiments/log.md) and the exit affordance was removed, so this event no longer fires; during the experiment it only fired for the `control` variant (events carry `$feature/onb-modal-exit` for the split), plus 2 locked-arm events from the flag-load race the bake-in deleted. Post-conclusion invariant: 0 new events |
