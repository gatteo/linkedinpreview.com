...
| `onb_paywall_gate_blocked` | `step: reveal\|paywall`                                  | the scroll-gated CTA was clicked before the reader reached the end; the click scrolls them there instead. The gate button is shared by the reveal and paywall steps, so only `step = paywall` counts purchase intent. Events from 2026-07-31 to 2026-08-07 carry no `step` and mix both screens (~42% were reveal) - never read them as paywall-only |
