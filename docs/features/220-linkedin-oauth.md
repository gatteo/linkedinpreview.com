...
- [x] 220-AC-2 The auth route requires an existing session and sets a CSRF state cookie before redirecting to LinkedIn consent _(verified: `app/api/linkedin/auth/route.ts:50-70` session check + `OAUTH_STATE_COOKIE` then `buildAuthorizeUrl(state)`)_
...
- [x] 220-AC-19 An OAuth round-trip started inside onboarding returns to a page that actually consumes the result, so the connected state reaches the flow _(verified: `components/dashboard/onboarding/onboarding-controller.tsx:303` sends `?from=onboarding`; `app/api/linkedin/auth/route.ts:65` sets `OAUTH_ORIGIN_COOKIE`; `app/api/linkedin/callback/route.ts:49-51` reads it and `oauthReturnRedirect` (`auth/route.ts:34-39`) sends onboarding statuses to `/dashboard` instead of `/dashboard/settings`. Fixes GH #30, where `LinkedInConnection`'s mount effect stripped `?linkedin` before the controller's async resume gate could read it and every OAuth user dead-looped at connect)_
...
- Return routing: the callback sends the member back to whichever page consumes the result. `?from=onboarding` on the auth route sets `OAUTH_ORIGIN_COOKIE`, and `oauthReturnRedirect` (`auth/route.ts:34`) routes the onboarding statuses (`ONBOARDING_LINKEDIN_STATUSES`, `config/linkedin.ts`) to `/dashboard`; everything else, including all account-switch outcomes, goes to `/dashboard/settings`. **Gotcha:** `/dashboard/settings` strips `?linkedin` in a mount effect, so any consumer whose read is gated behind async loads will lose the param there - that race was GH #30.
