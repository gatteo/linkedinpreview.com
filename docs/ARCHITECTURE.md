...
| GET    | /api/linkedin/callback    | OAuth callback: token exchange, then resolve account (attach / reconnect / login-switch / block). Fires `onb_oauth_callback` with the outcome when the round-trip started in onboarding | Session                           | `?code&state` (query)                                | 302 to Settings, or to `/dashboard` for onboarding-origin statuses (`?linkedin=...`) |
