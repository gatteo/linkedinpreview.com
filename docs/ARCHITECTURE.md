...
| POST   | /api/billing/webhook      | Stripe webhook: sync plan state into `billing` (service-role)                                    | Stripe signature                  | Stripe event payload                                 | `{ received: true }`                               |
