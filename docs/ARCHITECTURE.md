...
| BRIGHTDATA_API_KEY             | Bright Data "LinkedIn People Profile" dataset - the RICH onboarding tier (async 42-60s: posts, followers; powers the audit report). Triggered by the enrich route, polled via `enrich/status`.                                                            | No (server)               | (api key)                         |
