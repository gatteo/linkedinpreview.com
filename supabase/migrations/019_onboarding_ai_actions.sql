...
-- generation) in ai_usage, and backfill 'carouselGenerate' which config/ai.ts
-- already uses but no prior migration had added to the constraint.
...
alter table public.ai_usage add constraint ai_usage_action_check
    check (action in (
        'generation', 'refinement', 'analysis', 'wizard', 'quickAction',
